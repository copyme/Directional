...
namespace directional
{
  ...
  private:
    ...
    void treatSoftConstraints() {
      ...
      if (mBSoft.cols() == 3)  //N-RoSy constraint
      {
        for (unsigned int i = 0; i < mBSoft.rows(); i++) {
          std::complex<double> bComplex(mBSoft.row(i).dot(mB1.row(mBcSoft(i))),
                                        mBSoft.row(i).dot(mB2.row(mBcSoft(i))));
        ...
        for (unsigned int i = 0; i < mBSoft.rows(); i++) {
          ...
          for (unsigned int n = 0; n < N; n++) {
            ...
            roots(n) = mWSoft(i, n) *
                       std::complex<double>(vec.dot(mB1.row(mBcSoft(i))), vec.dot(mB2.row(mBcSoft(i))));
          }
          roots_to_monicPolynomial(roots, poly);
    ...
    unsigned int
    buildFullEnergyMatrix(const Eigen::MatrixXi &EV, const Eigen::MatrixXi &EF, OutputIterator result) {
    ...
    void evalNoConstraints(Eigen::MatrixXcd &polyVectorField) {
      ...
      polyVectorField.col(0) =
          U.block(0, 0, (long int) ((double) U.rows() / 2.), 1).cast<std::complex<double> >().array() *
          std::complex<double>(1., 0.) +
          U.block(int((double) U.rows() / 2.), 0, int((double) U.rows() / 2.),
                  1).cast<std::complex<double> >().array() * std::complex<double>(0., 1.);
    }
  ...
  public:
    ...
    void eval(Eigen::MatrixXcd &polyVectorField) {
      ...
      if (mSolver.info() != Eigen::Success)
        throw std::runtime_error(
            "directional::PolyVectorComputer::eval: Solving the system finished with a failure!");
